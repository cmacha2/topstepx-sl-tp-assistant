...
class NetworkInterceptor {
  constructor() {
    ...
    this.accountId = null; // Store account ID for line drag sync
  ...
  extractOrderData(data, action = 'create') {
    ...
    // Capture account ID for line drag sync
    if (data.accountId) {
      this.accountId = data.accountId;
      console.log('[TopstepX Network] 👤 Account ID captured:', this.accountId);
      
      // Notify line drag sync module if available
      if (typeof window.lineDragSync !== 'undefined') {
        window.lineDragSync.setAccountId(this.accountId);
      }
    }
