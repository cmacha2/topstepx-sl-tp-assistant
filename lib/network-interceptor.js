...
class NetworkInterceptor {
  constructor() {
    ...
    this.accountData = null; // Store current account balance info
    this.templateData = null; // Store template info (starting balance)
  ...
  processOrderRequest(url, options) {
    try {
      ...
      // DELETE = Cancel order
        ...
        // Extract order ID from URL: /Order/cancel/{accountId}/id/{orderId}
        ...
        if (orderIdMatch) {
          ...
          if (this.activeOrderId && this.activeOrderId.toString() === cancelledOrderId) {
            ...
            console.log('[TopstepX Network] ℹ️ Cancelled order is not the active order');
          }
        }
      }
  ...
  async processRequest(url, options, response) {
    try {
      ...
      // Capture TradingAccount data (for current balance)
      if (url && url.includes('userapi.topstepx.com/TradingAccount') && !url.includes('setPositionBrackets')) {
        try {
          const accountsData = await response.json();
          if (Array.isArray(accountsData) && accountsData.length > 0) {
            this.accountData = accountsData;
            console.log('[TopstepX Network] 💰 Account data captured:', accountsData.length, 'accounts');
            console.log('[TopstepX Network] 📊 First account balance:', accountsData[0].balance);
            this.notifyListeners('accountDataCaptured', { accounts: accountsData });
          }
        } catch (jsonError) {
          console.log('[TopstepX Network] ⚠️ Could not parse account data');
        }
      }

      // Capture AccountTemplate data (for starting balance)
      if (url && url.includes('userapi.topstepx.com/AccountTemplate/userTemplates')) {
        try {
          const templatesData = await response.json();
          if (Array.isArray(templatesData) && templatesData.length > 0) {
            this.templateData = templatesData;
            console.log('[TopstepX Network] 🎯 Template data captured:', templatesData.length, 'templates');
            console.log('[TopstepX Network] 📊 Templates:', templatesData.map(t => `${t.name}: $${t.startingBalance}`));
            this.notifyListeners('templateDataCaptured', { templates: templatesData });
          }
        } catch (jsonError) {
          console.log('[TopstepX Network] ⚠️ Could not parse template data');
        }
      }
    } catch (error) {
      // Ignore errors
  ...
  processXHRResponse(url, method, requestBody, responseText) {
    try {
      ...
      if (this.isOrderEndpoint(url)) {
        ...
        // Process for DELETE (cancel)
        if (method === 'DELETE' && url.includes('/Order/cancel/')) {
          const orderIdMatch = url.match(/\/id\/(\d+)/);
          if (orderIdMatch) {
            const cancelledOrderId = orderIdMatch[1];
            console.log('[TopstepX Network] ❌ XHR ORDER CANCELLED:', cancelledOrderId);
            
            // Check if this is the active order
            if (this.activeOrderId && this.activeOrderId.toString() === cancelledOrderId) {
              console.log('[TopstepX Network] 🗑️ Active order cancelled via XHR, clearing lines...');
              this.notifyListeners('orderCancelled', { orderId: cancelledOrderId });
              this.activeOrderId = null;
              this.orderData = null;
            } else {
              console.log('[TopstepX Network] ℹ️ Cancelled order is not the active order (XHR)');
            }
          }
        }
