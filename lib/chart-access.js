...
(function() {
  ...
  class ChartAccess {
    ...
    updateLines(slPrice, tpPrice, entryPrice, config, contracts, instrument) {
      ...
      this.persistToStore(slPrice, tpPrice, entryPrice, contracts, instrument, config);
    }
    ...
    detectLineDrag(currentSlPrice, currentTpPrice) {
      // Check if this is the first position check
     ...
     */
    persistToStore(slPrice, tpPrice, entryPrice, contracts, instrument, config) {
      if (typeof window.orderStore === 'undefined') {
      ...
      const orderData = {
        ...
        side: slPrice < entryPrice ? 'long' : 'short'
      };
