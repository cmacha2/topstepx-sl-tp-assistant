  ...
  class ChartAccess {
    constructor() {
      ...
      this.state = {
        ...
        instrument: null,
        config: null,
        side: null // Track side for persistence
      };
    ...
    updateLines(slPrice, tpPrice, entryPrice, config, contracts, instrument, side) {
      ...
      // Reset last positions to prevent false drag detection
      this.lastPositions = {
        slPrice: slPrice,
        tpPrice: tpPrice
      };
    ...
    detectLineDrag(currentSlPrice, currentTpPrice) {
      ...
      const slChanged = currentSlPrice !== null && 
                       ...
                       Math.abs(currentSlPrice - this.lastPositions.slPrice) > 0.01;
      
      const tpChanged = currentTpPrice !== null && 
                       this.lastPositions.tpPrice !== null && 
                       Math.abs(currentTpPrice - this.lastPositions.tpPrice) > 0.01;
      ...
      if (slChanged || tpChanged) {
        ...
        // Update OrderStore with new dragged positions (with debouncing)
        this.persistToStore(currentSlPrice, currentTpPrice, this.state.entryPrice, 
                           this.state.contracts, this.state.instrument, this.state.config, this.state.side);
      }
    }
