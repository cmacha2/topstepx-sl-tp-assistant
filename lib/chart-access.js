  ...
  class ChartAccess {
    ...
    handleLineDragged(lineType, newPrice) {
      try {
        ...
        const roundedPrice = this.roundToTick(newPrice, instrument.tickSize);
        
        // Calculate new dollar value
        const ticks = Math.abs(roundedPrice - entryPrice) / instrument.tickSize;
        const dollars = ticks * instrument.tickValue * contracts;
        
        // Update state
        if (lineType === 'SL') {
          this.state.slDollars = dollars;
          this.state.slPrice = roundedPrice;
        } else {
          this.state.tpDollars = dollars;
          this.state.tpPrice = roundedPrice;
        }
        
        console.log(`[TopstepX Chart] 💰 ${lineType} new value: $${dollars.toFixed(2)} @ ${roundedPrice}`);
