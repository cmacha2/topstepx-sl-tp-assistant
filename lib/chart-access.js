...
(function() {
  ...
  class ChartAccess {
     ...
     * @param {number} totalAccountValue - Total account value (starting balance + current balance)
     */
    formatLabel(type, dollars, contracts, config, totalAccountValue = null) {
      const prefix = type === 'sl' ? (config.slPrefix || 'SL') : (config.tpPrefix || 'TP');
      const sign = type === 'sl' ? '-' : '+';
      const emoji = config.useEmojis ? (type === 'sl' ? '🛑 ' : '🎯 ') : '';
      
      // If percentage mode is enabled and we have total account value
      if (config.showPercentage && totalAccountValue && totalAccountValue > 0) {
        // Calculate percentage: (dollars / totalAccountValue) * 100
        const percentage = (dollars / totalAccountValue) * 100;
        const percentStr = config.showDecimals ? 
          percentage.toFixed(2) : 
          percentage.toFixed(1); // Always show at least 1 decimal for percentages
        
        // Choose format
        switch (config.labelFormat) {
          case 'full':
            const fullName = type === 'sl' ? 'STOP LOSS' : 'TAKE PROFIT';
            return `${emoji}${fullName}: ${sign}${percentStr}%`;
          
          case 'minimal':
            return `${emoji}${prefix} ${sign}${percentStr}%`;
     ...
     * Update SL/TP lines
     */
    updateLines(slPrice, tpPrice, entryPrice, config, contracts, instrument, totalAccountValue = null) {
      ...
      console.log('[TopstepX Chart] 💰 Total Account Value:', totalAccountValue);
      ...
      this.state.totalAccountValue = totalAccountValue; // Save for dynamic updates
      ...
      const slLabel = this.formatLabel('sl', slDollars, contracts, config, totalAccountValue);
      const tpLabel = this.formatLabel('tp', tpDollars, contracts, config, totalAccountValue);
    ...
    updateLineLabels() {
      ...
      try {
        const { entryPrice, contracts, instrument, config, totalAccountValue } = this.state;
        ...
        if (slShape) {
          ...
          const newText = this.formatLabel('sl', slDollars, contracts, config, totalAccountValue);
        ...
        if (tpShape) {
          ...
          // Update text using configured format
          const newText = this.formatLabel('tp', tpDollars, contracts, config, totalAccountValue);
