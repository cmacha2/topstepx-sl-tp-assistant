...
const DEFAULT_CONFIG = {
  ...
  // Line Drag Sync
  enableLineDragSync: false,     // Auto-sync SL/TP to TopstepX when dragging lines
  syncDebounceDelay: 1000,       // Delay in ms before syncing after drag (debounce)
...
class StorageManager {
    ...
    return new Promise((resolve) => {
      if (typeof chrome !== 'undefined' && chrome.storage) {
        chrome.storage.sync.get(this.configKey, (result) => {
          ...
          console.log('[StorageManager] Config loaded and merged with defaults');
          resolve(mergedConfig);
        });
      } else {
        // Fallback for environments without chrome.storage
        console.log('[StorageManager] No chrome.storage, using defaults');
  ...
  async updateConfig(updates) {
    ...
    // Merge: defaults → saved config → updates
    const newConfig = { ...DEFAULT_CONFIG, ...config, ...updates };
    await this.saveConfig(newConfig);
    console.log('[StorageManager] Config updated:', updates);
    return newConfig;
  }
  ...
  async resetConfig() {
    ...
    console.log('[StorageManager] Config reset to defaults');
    return resetConfig;
  }
